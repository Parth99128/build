      ...
      return (
        <View style={{ flex: 1, backgroundColor: '#FEE2E2', padding: 40, justifyContent: 'center' }}>
          ...
          <ScrollView style={{ maxHeight: 400 }}>
            ...
            <Text style={{ fontSize: 11, color: '#991B1B', marginTop: 10 }}>{this.state.error.stack?.slice(0, 800)}</Text>
          </ScrollView>
        </View>
      );
    }
...
function AnalyticsStack({ onReset }: { onReset: () => void }) {
  return (
    <Stack.Navigator>
      <Stack.Screen name="AnalyticsMain" component={AnalyticsScreen} options={{ headerShown: false }} />
      <Stack.Screen name="SettingsPage" options={{ title: 'Settings', headerStyle: { backgroundColor: C.bg }, headerTintColor: C.pri, headerShadowVisible: false }}>
...
function MainTabs({ onReset }: { onReset: () => void }) {
  return (
      ...
      screenOptions={({ route }) => ({
        ...
        tabBarIcon: ({ focused, color }) => {
          const icons: Record<string, [string, string]> = {
            ...
            Deliveries: ['car', 'car-outline'],
            AnalyticsTab: ['analytics', 'analytics-outline'],
          };
    ...
    >
      ...
      <Tab.Screen name="AnalyticsTab" options={{ tabBarLabel: 'More' }}>
        {() => <AnalyticsStack onReset={onReset} />}
      </Tab.Screen>
    </Tab.Navigator>
  );
}
