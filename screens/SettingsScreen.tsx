...
export default function Settings({ onReset }: { onReset?: () => void }) {
  ...
  const exportJSON = async () => {
    ...
    try {
      ...
      await Share.share({ message: json, title: `BuildPro_Export_${new Date().toISOString().split('T')[0]}.json` });
    } catch {
  ...
  const exportFullCSV = async () => {
    ...
    try {
      ...
      await Share.share({ message: full, title: `BuildPro_FullReport_${new Date().toISOString().split('T')[0]}.csv` });
    } catch {
      setShowInfo({ visible: true, title: 'Error', message: 'Could not export data.', type: 'error' });
    }
  };
